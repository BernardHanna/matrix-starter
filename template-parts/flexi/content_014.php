<section class="relative w-full" style="background-color: <?php echo esc_attr(get_sub_field('background_color')); ?>;">
  <div
    class="flex flex-wrap items-start w-full gap-8 mx-auto max-w-[1084px] px-5 lg:px-4 py-8 lg:<?php echo get_sub_field('padding_y') ? 'py-[' . esc_attr(get_sub_field('padding_y')) . '%]' : 'py-12'; ?>">
    ...
    <div class="flex flex-col w-full lg:flex-row lg:justify-between max-w-[1084px] mx-auto">
      ...
      <div class="flex items-start gap-2 text-base w-full max-w-[599px]">
        <div class="flex flex-col w-full">
          <div class="mb-6 leading-6 text-black ">
            <?php echo wp_kses_post(get_sub_field('content_014_paragraph_text')); ?>
          </div>
          <?php if ($button = get_sub_field('content_014_button_link')): ?>
            <a href="<?php echo esc_url($button['url']); ?>"
              class="flex self-start gap-2 px-6 py-3 text-white rounded w-fit-content bg-secondary hover:bg-red hover:text-white hover:no-underline focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2"
